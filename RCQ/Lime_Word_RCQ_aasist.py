...
def run_dataset(model, protocol_path, audio_root, vad_dir, device="cuda",
    ...
    dataset_spoof_scores, dataset_bonafide_scores, dataset_tr_scores = [], [], []
    results_per_file = []
    ...
    for fname in tqdm(file_list, desc="Processing files", unit="file"):
        ...
        results_per_file.append({
            ...
            "mean_spoof": np.mean(spoof_scores) if spoof_scores else 0,
            "mean_bonafide": np.mean(bonafide_scores) if bonafide_scores else 0,
            "mean_tr": np.mean(tr_scores) if tr_scores else 0
        })

        if logger:
            logger.info(
                f"[DONE] {fname} (segments={len(segments)}, pred=spoof, "
                f"mean_spoof={np.mean(spoof_scores) if spoof_scores else 0:.4f}, "
                f"mean_bonafide={np.mean(bonafide_scores) if bonafide_scores else 0:.4f}, "
                f"mean_tr={np.mean(tr_scores) if tr_scores else 0:.4f})"
            )

    # dataset-level 평균 및 RCQ
    ...
    if save_csv:
        pd.DataFrame(results_per_file).to_csv(save_csv, index=False)
        if logger:
...
if __name__ == "__main__":
    ...
    logger.info("===== LIME RCQ Experiment Started (AASIST) =====")
